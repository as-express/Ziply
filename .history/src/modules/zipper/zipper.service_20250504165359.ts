...
export class ZipperService {
  ...
  async compressImages(image: any, req: Request) {
    const filePath = await this.fileService.writeFile(Type.IMAGES, image);
    const result = await compressImage(image);

    const item = await this.zipper.create({
      files: [filePath],
      size: result.size,
      sizeUnit: result.sizeUtil,
      userId: req.userId,
    });
